...
struct InformesTransitoCardView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            // Cabeçalho
            HStack(spacing: 12) {
                Image(systemName: "exclamationmark.triangle.fill").font(.system(size: 24)).foregroundColor(.white)
                Text(localizationManager.string(for: "traffic_reports"))
            ...
            if isLoading {
                ...
                EmptyStateView(
                    icon: "checkmark.seal",
                    message: localizationManager.string(for: "no_traffic_reports")
                )
            } else {
                // Carrossel Visível
                    ...
                    HStack {
                        ...
                        Button(action: {
                            withAnimation(.easeInOut) {
                                currentIndex = (currentIndex > 0) ? currentIndex - 1 : informes.count - 1
                            }
                        }) {
                        ...
                        Button(action: {
                            withAnimation(.easeInOut) {
                                currentIndex = (currentIndex < informes.count - 1) ? currentIndex + 1 : 0
                            }
                        }) {
...
// MARK: - Carrossel Horizontal (Com Gesto de Swipe)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ForEach(linhasProcessadas, id: \.self) { linha in
                if isAttentionLine(linha) {
                    ...
                    Text(linha).foregroundColor(.white)
