...
open class ARViewController: UIViewController, ARTrackingManagerDelegate
{
    ...
    fileprivate var setab: UIButton?
    ...
    open func setAnnotations(_ annotations: [ARAnnotation])
    {
        ...
        if self.annotations.count == 1 {
            self.setab = UIButton(frame: CGRect(x: (self.view.bounds.width/2)-32, y: self.view.bounds.height-100, width: 64, height:64))
            self.setab?.setImage(#imageLiteral(resourceName: "gps2.png"), for: .normal)
            self.view.addSubview(self.setab!)
        }
    ...
    open func reload(reloadType currentReload: ARViewController.ReloadType)
    {
        ...
        if self.annotations.count == 1 {
            let angle = (self.arStatus.heading.magnitude - self.annotations[0].azimuth)
            setab?.transform = CGAffineTransform(rotationAngle: (CGFloat(angle*(M_PI / 180.0))) * -1)
        }
    ...
    func addCloseButton()
    {
        ...
        if self.closeButtonImage == nil
        {
            ...
            let path = bundle.path(forResource: "x", ofType: "png")
            if let path = path
            {
